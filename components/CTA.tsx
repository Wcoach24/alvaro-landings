type Props = { cal_link?: string; cv_filename: string; cv_url?: string; company: string };

export default function CTA({ cal_link, cv_filename, cv_url, company }: Props) {
  const cvHref = cv_url || `/cv/${cv_filename}`;
  return (
    <section className="section section--hero" style={{ marginBottom: 28, padding: 28 }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
        ...
        <a href={cvHref} target={cv_url ? '_blank' : undefined} rel="noreferrer" download={!cv_url} className="btn btn--secondary">
          Download CV (PDF)
        </a>
